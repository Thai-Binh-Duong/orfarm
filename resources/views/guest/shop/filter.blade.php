                                    ...
                                    <ul >
                                        <li>
                                            <div class="checkbox-category" data-process="{{ route("shop.show") }}">
                                                <input class="checkbox-custom-feature" type="checkbox" name="categories[]" value="{{ $cate->id }}" id="category-{{ $cate->id }}" 
                                                @if ( !empty($categories_name) )
                                                    @foreach ($categories_name as $category_name)
                                                        @if ($category_name->id == $cate->id )
                                                            checked="checked"
                                                        @endif
                                                    @endforeach
                                                @endif
                                            > <label class="checkbox-important"  for="category-{{ $cate->id }}">{{ $cate->name }}</label> 
                                            </div>
...
{{-- 
                                            <ul class="child-filter">
                                                ...
                                                <li><input type="checkbox"  name="categories[]" value="{{ $child_category['id'] }}" id="category-child-{{ $child_category['id'] }}"
                                                    
                                                    > <label class="checkbox-important" for="category-child-{{  $child_category['id'] }}">{{  $child_category['name'] }}</label></li>
                            ...
                            <form action=" {{ route('shop.filter') }} " method="POST">
                                ...
                                <ul>
                                    @foreach ($brands as $brand)
                                        <li><input type="checkbox" name="brands[]" value="{{$brand->id}}" id="brand-child-{{$brand->id}}"
                                        @if ( !empty($brands_name) )
                                            @foreach ($brands_name as $brand_name)
                                                @if ($brand_name->id == $brand->id )
                                                    checked="checked"
                                                @endif
                                            @endforeach
                                        @endif
                                        
                                        ><label class="checkbox-important"  for="brand-child-{{$brand->id}}">{{$brand->brand_name}}<span>({{ $brand->products->count() }})</span></label></li>
                                    @endforeach
                                </ul>
            ...
            <div class="navigation">
                <div class="show">
                        ...
                        <span>Categories : 
                            ...
                            @foreach ( $categories_name as $category_name)
                                @php
                                    $count--;
                                @endphp
                                <span>{{ ucwords( $category_name->name ) }}@if ($count > 0)
                                    ,
                                @endif</span>
                            @endforeach 
                        </span>
                    ...
                    @if ( !empty($brands_name) )
                        <span>Brand : 
                            @php
                                $count = count($brands_name);
                            @endphp
                            @foreach ( $brands_name as $brand_name)
                                @php
                                    $count--;
                                @endphp
                                <span>{{ ucwords( $brand_name->brand_name ) }}@if ($count > 0)
                                    ,
                                @endif</span>
                            @endforeach 
                        </span>
                    ...
                    @if ( !empty($name_product) )
                        <span> Product Name : {{ ucwords( $brand_name->brand_name ) }} </span>
                    @endif
